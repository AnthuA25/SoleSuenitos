...
function OrdenProduccion() {
  ...
  useEffect(() => {
    const cargarRollos = async () => {
      try {
        ...
        Swal.fire("Error", "No se pudieron obtener los rollos desde el servidor.", "error");
      }
    };
    cargarRollos();
  }, []);
  ...
  const handleSubirMolde = async () => {
    ...
    Swal.fire({ title: "Analizando molde...", allowOutsideClick: false, didOpen: () => Swal.showLoading() });
    try {
      ...
      if (result?.piezas?.length) {
        ...
        Swal.fire("Sin piezas", "El archivo no contiene moldes válidos.", "info");
      }
    } catch {
      Swal.close();
  ...
  const handleVerRollos = async () => {
    ...
    const rows = rollosDisponibles
      .map((r, idx) => {
        const checked = rollosSeleccionados.includes(r.idRollo) ? "checked" : "";
        return `
          <tr>
    ...
    if (isConfirmed) {
      ...
      Swal.fire("Hecho", `${seleccion.length} rollo(s) seleccionados`, "success");
    }
  };
  ...
  const handleOptimizarCorte = async () => {
    ...
    Swal.fire({ title: "Generando V1...", allowOutsideClick: false, didOpen: () => Swal.showLoading() });
    ...
    try {
      ...
      rollosSeleccionados.forEach((r) => formData.append("rollosSeleccionados", r.idRollo));
      ...
      const metricas = result.data.metricas;
      setOptValues({
  ...
  const handleGenerarV2 = async () => {
    ...
    Swal.fire({ title: "Generando V2...", allowOutsideClick: false, didOpen: () => Swal.showLoading() });
  ...
  const handleComparar = async () => {
    Swal.fire({ title: "Comparando V1 y V2...", allowOutsideClick: false, didOpen: () => Swal.showLoading() });
    try {
      const formData = new FormData();
      formData.append("modelo", modelo);
      formData.append("talla", talla);
      ...
      Swal.close();
      Swal.fire("Comparación completada", `La mejor versión es: <b>${result.mejor_version}</b>`, "success");
          ...
          {!showOptimizacion && !showMarcadorV2 && !showCompararMarcadores ? (
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 14 }}>
                <div>
                  <label>Modelo</label>
                  <input className="orden-input" value={modelo} onChange={(e) => setModelo(e.target.value)} placeholder="Ej. Pijama de invierno" />
                </div>
                <div>
                  <label>Talla</label>
                  <input className="orden-input" value={talla} onChange={(e) => setTalla(e.target.value)} placeholder="Ej. S" />
                </div>
                <div>
                  <label>Cantidad</label>
                  <input className="orden-input" type="number" value={cantidad} onChange={(e) => setCantidad(e.target.value)} placeholder="Ej. 10" />
                </div>
                <div>
                  <label>Rollos disponibles</label>
                  <button className="elegir-btn" onClick={handleVerRollos} style={{ width: "100%", marginTop: 8 }}>
                    Ver Rollos
                  </button>
                  <div style={{ marginTop: 10, display: "flex", gap: 8, flexWrap: "wrap" }}>
                    {rollosSeleccionados.length === 0
                      ...
                      : rollosSeleccionados.map((r) => (
                          <div key={r.idRollo} className="rollo-tag">{r.codigoRollo}</div>
                        ))}
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
                <button className="subir-btn" onClick={handleSubirMolde}>Subir molde</button>
              </div>
              ...
              <div style={{ background: "#fff", borderRadius: 8, padding: 12, marginBottom: 12 }}>
                <table className="orden-table">
                  ...
                  <tbody>
                    {moldes.length === 0 ? (
                      <tr><td colSpan="5" style={{ textAlign: "center", color: "#888" }}>Sin piezas cargadas</td></tr>
                    ) : (
                      moldes.map((m, i) => (
                        <tr key={i}>
                          <td>{m.pieza}</td>
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 12 }}>
                  <button className="cancelar-btn" onClick={handleVolverOrden}>Cancelar</button>
                  <button className="optimizar-btn" onClick={handleOptimizarCorte}>Optimizar Corte</button>
                </div>
              </div>
            </>
          ) : showOptimizacion ? (
            <>
              ...
              <div className="optimizacion-box">
                <table>
                  <tbody>
                    <tr><td><b>Tela utilizada:</b></td><td>{optValues.telaUtilizada}</td></tr>
                    <tr><td><b>Aprovechamiento:</b></td><td>{optValues.aprovechamiento}</td></tr>
                    <tr><td><b>Desperdicio:</b></td><td>{optValues.desperdicio}</td></tr>
                  </tbody>
                </table>
                <div style={{ display: "flex", justifyContent: "center", gap: 10, marginTop: 20 }}>
                  <button className="optimizar-btn" onClick={handleGenerarV2}>Generar V2</button>
                  <button className="cancelar-btn" onClick={handleVolverOrden}>Volver</button>
                </div>
              </div>
            </>
          ) : showMarcadorV2 ? (
            <>
              ...
              <div className="optimizacion-box">
                <table>
                  <tbody>
                    <tr><td><b>Tela utilizada:</b></td><td>{optValuesV2?.telaUtilizada}</td></tr>
                    <tr><td><b>Aprovechamiento:</b></td><td>{optValuesV2?.aprovechamiento}</td></tr>
                    <tr><td><b>Desperdicio:</b></td><td>{optValuesV2?.desperdicio}</td></tr>
                  </tbody>
                </table>
                <div style={{ display: "flex", justifyContent: "center", gap: 10, marginTop: 20 }}>
                  <button className="optimizar-btn" onClick={handleComparar}>Comparar Marcadores</button>
                  <button className="cancelar-btn" onClick={handleVolverOrden}>Volver</button>
                </div>
              </div>
            </>
